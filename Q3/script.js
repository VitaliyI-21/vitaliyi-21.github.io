...
hintButton.addEventListener('click', () => {
  ...
  if (!hintText) {
    ...
    hintText.textContent = 'Підсказка: створи h2 з текстом "CSS важливий" і задай йому синій колір у CSS.';
...
checkButton.addEventListener('click', () => {
  ...
  const colorMatch = css.match(/h2\s*{[^}]*color\s*:\s*(#[0-9a-fA-F]{3,6}|blue);?/);

  if (h2 && h2.textContent.trim().toLowerCase() === 'css важливий' && colorMatch) {
    ...
    if (!nextBtn) {
      ...
      nextBtn.addEventListener('click', () => {
        ...
        if (match) {
          ...
          window.location.href = '../Q4/Q4.html';
        }
      });
    }
  } else {
