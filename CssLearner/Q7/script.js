...
hintButton.addEventListener('click', () => {
  ...
  if (!hintText) {
    ...
    hintText.textContent = 'Підсказка: створи <div class="highlight">...</div> і в CSS напиши .highlight { background-color: yellow; }';
...
checkButton.addEventListener('click', () => {
  ...
  const backgroundMatch = css.match(/\.highlight\s*{[^}]*background\s*-?color\s*:\s*(yellow|#ffff00)/i);

  if (div && backgroundMatch) {
    ...
    if (!nextBtn) {
      ...
      nextBtn.addEventListener('click', () => {
        ...
        if (match) {
          ...
          window.location.href = '../Q8/Q8.html';
        }
      });
    }
  } else {
