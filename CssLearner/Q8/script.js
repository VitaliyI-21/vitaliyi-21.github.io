...
hintButton.addEventListener('click', () => {
  ...
  if (!hintText) {
    ...
    hintText.textContent = 'Підсказка: використовуй тег <hr> і задай йому червоний колір через властивість border або background.';
...
checkButton.addEventListener('click', () => {
  ...
  const redMatch = css.match(/hr\s*{[^}]*((border(-top)?|background(-color)?)\s*:\s*red|#f{3}|#ff0000)/i);
    ...
    if (!nextBtn) {
      ...
      nextBtn.addEventListener('click', () => {
        ...
        if (match) {
          ...
          window.location.href = '../Q9/Q9.html';
        }
      });
    }
  } else {
