...
hintButton.addEventListener('click', () => {
  ...
  if (!hintText) {
    ...
    hintText.textContent = 'Підсказка: створи тег <button> з id "myButton"';
...
checkButton.addEventListener('click', () => {
  ...
  const btn = doc.querySelector('button#myButton');

  if (btn && btn.textContent.trim().toLowerCase() === 'натисни мене') {
    ...
    if (!nextBtn) {
      ...
      nextBtn.addEventListener('click', () => {
        ...
        if (match) {
          ...
          window.location.href = '../Q6/Q6.html';
        }
      });
    }
  } else {
