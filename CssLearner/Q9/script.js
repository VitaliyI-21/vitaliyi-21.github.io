...
hintButton.addEventListener('click', () => {
  ...
  if (!hintText) {
    ...
    hintText.textContent = 'Підсказка: CSS-коментар пишеться як /* коментар */';
...
checkButton.addEventListener('click', () => {
  const css = cssCode.value;
  const commentMatch = css.match(/\/\*\s*це мій перший стиль\s*\*\//i);
    ...
    if (!nextBtn) {
      ...
      nextBtn.addEventListener('click', () => {
        ...
        if (match) {
          ...
          window.location.href = '../Q10/Q10.html';
        }
      });
    }
  } else {
