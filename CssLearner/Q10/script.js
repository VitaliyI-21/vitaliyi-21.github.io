...
hintButton.addEventListener('click', () => {
  ...
  if (!hintText) {
    ...
    hintText.textContent = 'Підсказка: створи <html>, <head>, <body>, <h1> і в CSS для h1 задай font-family: Arial;';
...
checkButton.addEventListener('click', () => {
  ...
  const fontMatch = css.match(/h1\s*{[^}]*font-family\s*:\s*Arial/i);

  if (hasHtml && hasHead && hasBody && hasH1 && fontMatch) {
    ...
    if (!nextBtn) {
      ...
      nextBtn.addEventListener('click', () => {
        ...
        if (match) {
          ...
          window.location.href = '../Q11/Q11.html';
        }
      });
    }
  } else {
