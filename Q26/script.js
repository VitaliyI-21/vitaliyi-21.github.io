...
hintButton.addEventListener('click', () => {
  ...
  if (!hintText) {
    ...
    hintText.textContent = 'Підсказка: .spaced { margin-top: 20px; margin-bottom: 20px; }';
...
checkButton.addEventListener('click', () => {
  ...
  const heading = doc.querySelector('h2.spaced');
  const topMatch = css.match(/\.spaced\s*{[^}]*margin-top\s*:\s*20px/i);
  const bottomMatch = css.match(/\.spaced\s*{[^}]*margin-bottom\s*:\s*20px/i);

  if (heading && topMatch && bottomMatch) {
    ...
    if (!nextBtn) {
      ...
      nextBtn.addEventListener('click', () => {
        ...
        if (match) {
          ...
          window.location.href = '../Q27/Q27.html';
        }
      });
    }
  } else {
