...
hintButton.addEventListener('click', () => {
  ...
  if (!hintText) {
    ...
    hintText.textContent = 'Підсказка: використовуй <ul> з трьома <li> елементами: HTML, CSS, JavaScript';
...
checkButton.addEventListener('click', () => {
  ...
  if (ul) {
    const items = Array.from(ul.querySelectorAll('li')).map(li => li.textContent.trim().toLowerCase());
    const required = ['html', 'css', 'javascript'];
    const valid = required.every(r => items.includes(r));
      ...
      if (!nextBtn) {
        ...
        nextBtn.addEventListener('click', () => {
          ...
          if (match) {
            ...
            window.location.href = '../Q5/Q5.html';
          }
        });
      }
      return;
